    ...
    private class FtdiSerialPort extends CommonUsbSerialPort {
        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            ...
            synchronized (mReadBufferLock) {
                ...
                return filterStatusBytes(mReadBuffer, dest, totalBytesRead, mReadEndpoint.getMaxPacketSize());
            }
        }
         ...
         */
        @Override
        public int read(ByteBuffer target, int timeoutMillis) throws IOException {
            final int readAmt = target.remaining();
            final UsbRequest request = new UsbRequest();
            request.initialize(mConnection, mReadEndpoint);

            if (!request.queue(target, readAmt)) {
                throw new IOException("Error queueing request.");
            }

            if( mConnection.requestWait() == request){
                final int payloadBytesRead = target.position() - MODEM_STATUS_HEADER_LENGTH;
                if (payloadBytesRead > 0) {
                    //Log.d(TAG, HexDump.dumpHexString(target.array(), 0, Math.min(32, target.limit())));
